...
/*
 ...
 -- Which keyword we are using in JAVAV to filter
    -- if statement


 -- To filter result from database while we are running query we are gonna use WHERE keyword
...
-- display all information from employees where salary bigger than 6000
select * from employees
 where SALARY>6000;
...
-- display all information from employees who is making more tan 6000 and department id is 60;
select * from employees
...
-- display all info from employees_id between 100 and 300
select * from EMPLOYEES
...
-- IN CLAUSES --
-- it uses or logic
-- it checks from IN parenthesis to see condition is matching
-- solution 2
select * from EMPLOYEES
...
-- display all information names except country id is US and IT
...
-- IS NOT NULL
-- dispaly all information from employees where department id is not null
select * from EMPLOYEES
